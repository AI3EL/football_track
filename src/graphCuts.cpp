...
void testGCuts() {
    ...
    for (
        ...
        if (g.
                what_segment(i)
            == Graph<int, int, int>::SOURCE)
            cout << i << " is in the SOURCE set" <<
                 endl;
        else
            cout << i << " is in the SINK set" <<
                 endl;
}
...
int fish() {
    ...
    for (int i = 0; i < I.width(); i++) {
        for (int j = 0; j < I.height(); j++) {
            ...
            // Inter nodes weights :
            if (i > 0)
                g.add_edge(I.height() * i + j, I.height() * (i - 1) + j, curve_weight(I(i, j), I(i - 1, j)),
                           curve_weight(I(i, j), I(i - 1, j)));
            if (i < I.width() - 1)
                g.add_edge(I.height() * i + j, I.height() * (i + 1) + j, curve_weight(I(i, j), I(i + 1, j)),
                           curve_weight(I(i, j), I(i + 1, j)));
            if (j > 0)
                g.add_edge(I.height() * i + j, I.height() * i + j - 1, curve_weight(I(i, j), I(i, j - 1)),
                           curve_weight(I(i, j), I(i, j - 1)));
            if (j < I.height() - 1)
                g.add_edge(I.height() * i + j, I.height() * i + j + 1, curve_weight(I(i, j), I(i, j + 1)),
                           curve_weight(I(i, j), I(i, j + 1)));
        }
